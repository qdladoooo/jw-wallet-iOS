...
- (void)addCollectionPage{
    ...
    line.frame = CGRectMake(0, self.pageBarHeight - 5, self.lineWidth, 5);
    [self.contentViews addSubview:line];
    [self.contentViews bringSubviewToFront:line];
    self.line = line;
    
    UIView *BottomLIne = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.contentViews.frame)-1, SCREEN_Width, 1)];
    BottomLIne.backgroundColor = [UIColor colorFromHexRGB:@"f5f5f5"];
    [self.contentViews addSubview:BottomLIne];
}
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.line.frame = CGRectMake((0 + indexPath.row) * self.lineWidth, self.pageBarHeight - 5, self.lineWidth, 5);
    }];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.line.frame = CGRectMake(index *self.lineWidth, self.pageBarHeight - 5, self.lineWidth, 5);
    }];
...
- (void)pageContentView:(LZPageContentView *)pageContentView didClickBtnIndex:(NSInteger)index
{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        self.line.frame = CGRectMake((0 + index) *self.lineWidth, self.pageBarHeight - 5, self.lineWidth, 5);
    }];
}
